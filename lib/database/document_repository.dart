...
import '../models/flashcard_with_document.dart';
...
class DocumentRepository {
  ...
  // ============================================================
  // GET FLASHCARDS FOR ONE DOCUMENT
  // ============================================================
  ...
  Future<List<FlashcardWithDocument>> getAllFlashcards() async {
    print("LOADING ALL FLASHCARDS");

    final rows = await db.select(db.flashcards).get();

    final results = <FlashcardWithDocument>[];
    ...
    for (final row in rows) {
      ...
      results.add(
        FlashcardWithDocument(documentId: row.documentId, deck: deck),
      );
    }

    print("ALL FLASHCARDS LOADED");
    print("Deck count: ${results.length}");
